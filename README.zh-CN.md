...
- 支持 PVZ3 风格的完成后加料机制
- 满瓶屏蔽策略会单独保留可锁死加料的布局（尤其是恰好 4 个可接料瓶），再将其完整后续成本与立即清瓶及其他布局比较
...
内置示例的 2,048 个固定补料种子测试中，所有策略均为 100% 通关。新策略在第一次清瓶前多布局 1 步，留下恰好 4 个可接料瓶，把该次随机加料锁成唯一结果；最终平均为 19.208 步，观测最坏值为 20 步。相较平均 20.844 步、最坏 23 步的最快清瓶基线，平均减少 1.635 步（7.85%）；相较上一版平均 20.598 步、最坏 22 步的完整策略，平均再减少 1.390 步（6.75%）。盲目追求更多满瓶护盾仍会升至平均 23 步，因此算法只会在完整后续成本确实更低时优先选择锁死加料。
